...
from src.onion.onion_divergence import OnionDivergence
...
def parallel_computation(input_path, output_path, dataset, model):
    ...
    output_filename = f'{os.path.join(output_path,model)}/onion_{dataset}_{model}.csv'
    # if the data already exists, just don't redo it.
    if os.path.isfile(output_filename):
        df = pd.read_csv(output_filename, dtype={'model': str, 'gen': int, 'trial_id': int, 'onion': float}, sep='\t')
        df['name'] = [dataset]*df.shape[0]

        df.to_csv(f'{os.path.join(output_path,model)}/onion_{dataset}_{model}.csv', float_format='%.7f', sep='\t', index=False, na_rep='nan')
    ...
    for root, trial in tqdm(load_data(input_path, dataset, model), desc=model+dataset):
        ...
        for i, desc_graph in enumerate(descendants, 1):
            od = OnionDivergence()
            d = od.dist(G1=root_graph, G2=desc_graph)
...
if __name__ == '__main__':
    ...
    cols = ['name', 'model', 'trial_id', 'gen', 'onion']
    parallel_args = []
    results = []
    ...
    df.to_csv(f'{output_path}/merged_onion.csv', float_format='%.7f', sep='\t', index=False, na_rep='nan')