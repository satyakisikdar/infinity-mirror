...
def sample_tensor(y,sample=True, thresh=0.5):
    # do sampling
    if sample:
        y_thresh = (torch.rand(y.size())).cpu()
        y_result = torch.gt(y,y_thresh).float()
    # do max likelihood based on some threshold
    else:
        y_thresh = (torch.ones(y.size())*thresh).cpu()
        y_result = torch.gt(y, y_thresh).float()
    return y_result
...
def sample_sigmoid(y, sample, thresh=0.5, sample_time=2):
    ...
    if sample:
        if sample_time>1:
            y_result = (torch.rand(y.size(0),y.size(1),y.size(2))).cpu()
            # loop over all batches
            for i in range(y_result.size(0)):
                # do 'multi_sample' times sampling
                for j in range(sample_time):
                    y_thresh = (torch.rand(y.size(1), y.size(2))).cpu()
            ...
            y_thresh = (torch.rand(y.size(0),y.size(1),y.size(2))).cpu()
            y_result = torch.gt(y,y_thresh).float()
    # do max likelihood based on some threshold
    else:
        y_thresh = (torch.ones(y.size(0), y.size(1), y.size(2))*thresh).cpu()
        y_result = torch.gt(y, y_thresh).float()
    return y_result
...
def sample_sigmoid_supervised(y_pred, y, current, y_len, sample_time=2):
    ...
    # do sampling
    y_result = (torch.rand(y_pred.size(0), y_pred.size(1), y_pred.size(2))).cpu()
    # loop over all batches
    for i in range(y_result.size(0)):
        # using supervision
        if current<y_len[i]:
            while True:
                y_thresh = (torch.rand(y_pred.size(1), y_pred.size(2))).cpu()
...
class LSTM_plain(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return ((torch.zeros(self.num_layers, batch_size, self.hidden_size)).cpu(),
                (torch.zeros(self.num_layers, batch_size, self.hidden_size)).cpu())
...
class MLP_VAE_plain(nn.Module):
    ...
    def forward(self, h):
        ...
        eps = (torch.randn(z_sgm.size())).cpu()
        z = eps*z_sgm + z_mu
        # decoder
        y = self.decode_1(z)
        y = self.relu(y)
...
class MLP_VAE_conditional_plain(nn.Module):
    ...
    def forward(self, h):
        ...
        # reparameterize
        z_sgm = z_lsgms.mul(0.5).exp_()
        eps = (torch.randn(z_sgm.size(0), z_sgm.size(1), z_sgm.size(2))).cpu()
        z = eps * z_sgm + z_mu
        # decoder
...
def message_passing(node_neighbor, node_embedding, model):
    ...
    for i in range(len(node_neighbor)):
        ...
        if neighbor_num > 0:
            ...
            message_null = (torch.zeros((node_embedding[i].size(0),node_embedding[i].size(1)*2))).cpu()
...
class Graph_RNN_structure(nn.Module):
    def __init__(self, hidden_size, batch_size, output_size, num_layers, is_dilation=True, is_bn=True):
        ...
        # when using convolution to compute attention weight, we need to first concat the list into a pytorch : batch*hidden_dim*current_num_nodes
        self.hidden_all = []
    ...
    def init_hidden(self,len=None):
        if len is None:
            return (torch.ones(self.batch_size, self.hidden_size, 1)).cpu()
        else:
            hidden_list = []
            for i in range(len):
                hidden_list.append((torch.ones(self.batch_size, self.hidden_size, 1)).cpu())
            return hidden_list
    ...
    def forward(self, x, teacher_forcing, temperature = 0.5, bptt=True,bptt_len=20, flexible=True,max_prev_node=100):
        ...
        # # # add BPTT, detach the first 
        # if bptt:
        #     self.hidden_all[0] = (self.hidden_all[0].data).cpu()
        ...
        #     y_pred = (torch.zeros(x.size(0), x.size(1), x.size(2))).cpu()
        #     y_pred_long = (torch.zeros(x.size(0), x.size(1), x.size(2))).cpu()
        #     x_step = x[:, 0:1, :]
        ...
        thresh = 0.5
        x_thresh = (torch.ones(x_pred_sample.size(0), x_pred_sample.size(1), x_pred_sample.size(2)) * thresh).cpu()
...
# batch_size = 8
# output_size = 4
# generator = Graph_RNN_structure(hidden_size=16, batch_size=batch_size, output_size=output_size, num_layers=1).cpu()
# for i in range(4):
#     generator.hidden_all.append(generator.init_hidden())
#
# x = (torch.rand(batch_size,1,output_size)).cpu()
...
class Graph_generator_LSTM(nn.Module):
    ...
    def init_hidden(self):
        return ((torch.zeros(self.num_layers,self.batch_size, self.hidden_size)).cpu(), (torch.zeros(self.num_layers,self.batch_size, self.hidden_size)).cpu())
    ...
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        for m in self.modules():
            if isinstance(m, GraphConv):
                ...
                # m.weight.data = torch.rand([m.input_dim, m.output_dim]).cpu()*init_range
                # print('find!')
...
# x = (torch.rand(1,8,10)).cpu()
# adj = (torch.rand(1,8,8)).cpu()
# model = GCN_encoder_graph(10,10,10).cpu()
# y = model(x,adj)
# print(y.size())
...
def preprocess(A):
    ...
    D = (torch.zeros(A.size(0),A.size(1),A.size(2))).cpu()
...
class GCN_generator(nn.Module):
    ...
    def forward(self,x,teacher_force=False,adj_real=None):
        ...
        batch_num = x.size(0)
        node_num = x.size(1)
        adj = (torch.eye(node_num).view(1,node_num,node_num).repeat(batch_num,1,1)).cpu()
        adj_output = (torch.eye(node_num).view(1,node_num,node_num).repeat(batch_num,1,1)).cpu()
        ...
        for i in range(1, node_num):
            ...
            # x = x / torch.norm(x, p=2, dim=2, keepdim=True)
        # one = (torch.ones(adj_output.size(0), adj_output.size(1), adj_output.size(2)) * 1.00).cpu().float()
        # two = (torch.ones(adj_output.size(0), adj_output.size(1), adj_output.size(2)) * 2.01).cpu().float()
...
#             x = (torch.rand(batch,i,4)).cpu()
#             adj = (torch.eye(i).view(1,i,i).repeat(batch,1,1)).cpu()
#             # print('x', x)
#             # print('adj', adj)
#
#             # y = generator(x)
...
#### test code ####
# x = (torch.randn(1, 256, 1)).cpu()
# decoder = CNN_decoder(256, 16).cpu()
# y = decoder(x)
...
class Graphsage_Encoder(nn.Module):
    ...
    def forward(self, nodes_list, nodes_count_list):
        ...
        nodes_features_farther = (torch.Tensor(nodes_features.size(0), nodes_count.size(1), nodes_features.size(2))).cpu()
        i = 0
